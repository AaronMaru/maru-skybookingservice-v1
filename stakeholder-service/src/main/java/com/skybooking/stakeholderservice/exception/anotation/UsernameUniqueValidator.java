...
public class UsernameUniqueValidator implements ConstraintValidator<UsernameUnique, Object> {
    ...
    public boolean isValid(final Object value, final ConstraintValidatorContext context) {
        ...
        try
        {
            ...
            Boolean n = Boolean.parseBoolean(userRepository.existsByEmailOrPhone(username, code, lastSeg.equals("company") ? userBean.getUserPrincipalVld().getId() : null));
            Boolean b = Boolean.parseBoolean(contactRP.existsContact(NumberUtils.isNumber(username) ? code + "-" + username : username, lastSeg.equals("company") ? userBean.getUserPrincipalVld().getStakeHolderUser().getStakeholderCompanies().stream().findFirst().get().getId() : null));
